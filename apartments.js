'use strict';
const properties={pca:{name:'Prishtina City Apartments',location:'Prishtina · City center'},pca2:{name:'Prishtina City Apartments 2',location:'Prishtina · Nartel'}};
const propertyKey=new URLSearchParams(location.search).get('property')==='pca2'?'pca2':'pca';
...
document.title=property.name+' — Sfera';
...
document.querySelector(`[data-property="${propertyKey}"]`).setAttribute('aria-current','page');
...
async function openRoom(room){
  ...
  document.getElementById('group-note').hidden=!room.name.includes('&');
  document.getElementById('room-enquiry').href='https://wa.me/38348101070?text='+encodeURIComponent('Hello Sfera, I would like to check availability for '+room.name+' at '+property.name+'.');
